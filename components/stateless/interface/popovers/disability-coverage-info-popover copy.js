...
const DisabilityCoverageInfoPopover = ({ selectedKey, client, anchorEl, handleClose }) => {
  ...
  return (
    <BaseCoverageInfoPopover anchorEl={anchorEl} handleClose={handleClose}>
      ...
      <ClientCardInfoText
        ...
        value={`$${formatNumber(client.financialOverview[selectedKey].currentCoverage)}/month`}
      />
      <ClientCardInfoText
        xs={12}
        sm={12}
        label="Ideal Coverage"
        value={`$${formatNumber(client.financialOverview[selectedKey].idealCoverage)}/month`}
      />
    </BaseCoverageInfoPopover>
  );
};

export default DisabilityCoverageInfoPopover;
